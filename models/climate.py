...
HUMIDITY_DATA = (79.3, 79.8, 80.3, 83.0, 85.8, 89.8, 88.9, 86.2, 81.3, 78.5, 77.2, 77.6)
HUMIDITY_FUNCTION = lambda t, a, c, d: a*np.sin(2*np.pi*t-c)+d
COEFS, _ = optimize.curve_fit(HUMIDITY_FUNCTION,
                              np.linspace(0, 1, 12), HUMIDITY_DATA, p0=[10, 0, 80])
DQ = lambda t: 2*COEFS[0]*np.pi*np.cos(2*np.pi*t-COEFS[2])
...
def model(t, state, parameters):
    ...
    lm = rmin + (rmax-rmin)*np.exp(-a*q/100)
    return [
